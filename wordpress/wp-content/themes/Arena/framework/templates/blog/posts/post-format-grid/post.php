...
<li id="post-<?php the_ID(); ?>" <?php post_class(' post-grid-item '.$class ); ?>>
  <div class="inner-content">
    ...
    <?php if( is_sticky() ){ echo '<div class="sticky-post"><span>'. __('Sticky Post','brad') .'</span></div>';}?>
    
    <h4><a href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__('Permalink to %s', 'brad'), the_title_attribute('echo=0') ); ?>"><?php the_title(); ?></a></h4>
    ...
    <?php 
        ...
        <?php if($brad_data['check_author'] || $brad_data['check_blog_categories'] ||  $brad_data['check_readmore']  ||  $brad_data['check_blog_date'] || (comments_open() && $brad_data['check_blog_comments']) || $brad_data['check_postlove'] || $brad_data['check_blogshare'] == true):?>
