<?php
	 ...
	 if($style == 'style3'){
	 $output .= "#feature_boxes_{$fbox_id} .span .inner-content{padding:". intval($ip_top). "px ". intval($ip_left). "px ". intval($ip_top). "px ". intval($ip_left). "px ;";
	 ...
	 $css_class =  apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, 'row-fluid '.$style.' background-shadow-'.$bg_shadow.' feature_boxes box-'.$box_style.' enable-hr-'.$divider.' element-vpadding-'.$vpadding.' hr-type-'.$di_type.' hr-color-'.$di_color.' hr-style-'.$di_style.' '.$icon_size.'-size  icon-side-'.$icon_side.'  iconbox-'.$icon_style.'  align-content-center-'. $fc_align.' align-icon-center-'.$fi_align.' columns-'.$columns.' crease-background-'.$enable_crease.' content-box '.$color_scheme.' element-padding-'.$padding.' '.$el_class.' ', $this->settings['base']);
