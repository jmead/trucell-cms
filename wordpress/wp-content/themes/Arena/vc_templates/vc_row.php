<?php
	...
	if ($this->settings['base']=='vc_row_inner') :
	  ...
	  $style .= "padding-top:{$sp_top};padding-bottom:{$sp_bottom};margin:{$mtop}px {$mright}px {$mbottom}px {$mleft}px;";
	  ...
	  $cstyle = '';
	  if( $section_type == 'full-width-with-padding'){
	    $sp_left = ($sp_left == '0' || $sp_left == 0 ) ? 0 : $sp_left.'px';
	    $sp_right = ($sp_right == '0' || $sp_right == 0 ) ? 0 : $sp_right.'px'; 
	    $cstyle = "style='padding-left:{$sp_left};padding-right:{$sp_right};'";
	  }
	  
	  $output .= '<div class="container section-content" '.$cstyle.'><div class="row-fluid"><div class="row-fluid equal-cheight-'.$equal_height.'  element-padding-'.$padding.' element-vpadding-'.$vpadding.'">';
	  $output .= wpb_js_remove_wpautop($content);
